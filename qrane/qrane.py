...
def run_pre_flight_checks(path_manager: PathManager, ui=None) -> bool:
    try:
        if not (path_manager.root / 'pipeline_config.yaml').exists():
            raise FileNotFoundError("pipeline_config.yaml missing")
        ...
        return False
    # (Provider Checks - abbreviated for brevity but assumed present)
    return True
...
def handle_cheqpoint(cycle: int, args, reqap_path: Path, prefix: str, path_manager: PathManager, ui=None) -> str:
    ...
    while True:
        ...
        if choice == 'q':
            ...
            try: with open(reqap_path, 'r', encoding='utf-8') as f: content = f.read()
            except: pass
            continue
