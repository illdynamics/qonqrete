...
class QonQreteRunner {
    ...
    async hasTasqFile(preferredFolder) {
        ...
        if (rootTasq && fs.existsSync(rootTasq))
            return true;
     ...
     */
    async getRootTasqPath(preferredFolder) {
     ...
     */
    async getInternalTasqPath(preferredFolder) {
        const workingDir = await this.getQonQreteWorkingDir(preferredFolder);
        return workingDir ? path.join(workingDir, 'worqspace', 'tasq.md') : undefined;
    }
    /**
     ...
     */
    async isDeployed(preferredFolder) {
        const wsFolder = preferredFolder || vscode.workspace.workspaceFolders?.[0];
        if (!wsFolder)
            return false;
     ...
     */
    async syncRootTasqToInternal(preferredFolder) {
        const rootTasq = await this.getRootTasqPath(preferredFolder);
        const internalTasq = await this.getInternalTasqPath(preferredFolder);
        if (!rootTasq || !internalTasq)
            return false;
     ...
     */
    async ensureGitignore(preferredFolder) {
        const wsFolder = preferredFolder || vscode.workspace.workspaceFolders?.[0];
        if (!wsFolder)
            return;
    ...
    buildRunCommand(config, taskFilePath) {
    ...
    async run(config, preferredFolder) {
        ...
        this.outputChannel.appendLine(`[QonQrete] Config: sens=${config.sensitivity}, cycles=${config.cycles}, mode=${config.mode}, auto=${config.autonomous}, noSync=${config.noSync}`);
