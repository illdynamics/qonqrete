...
exports.QonQreteSidebarProvider = void 0;
const vscode = __importStar(require("vscode"));
const qonqreteRunner_1 = require("../cli/qonqreteRunner");
class QonQreteSidebarProvider {
    static viewType = 'qonqreteControlPanel';
    _view;
    _extensionUri;
    _runStateDisposable;
    ...
    resolveWebviewView(webviewView, _context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (data) => {
            switch (data.type) {
                ...
                case 'openFile':
                    ...
                    break;
                case 'showOutput':
                    (0, qonqreteRunner_1.getRunner)().showOutput();
                    break;
                case 'installBash':
        ...
        this._runStateDisposable = (0, qonqreteRunner_1.getRunner)().onRunStateChange((status) => {
            this._sendRunState(status);
        });
        this._sendStatus();
    }
    async _handleRunTasq(config) {
    ...
    async _sendQageList() {
        ...
        const runner = (0, qonqreteRunner_1.getRunner)();
        const qages = await runner.getAvailableQages();
        const qageDetails = await Promise.all(qages.slice(0, 10).map(async (name) => {
    ...
    async _sendQageDetails(qageName) {
        if (!this._view)
            return;
        const runner = (0, qonqreteRunner_1.getRunner)();
        ...
        <div class="section">
            ...
            <div class="form-group">
                ...
                <div class="slider-row">
                    <input type="range" id="sensitivity" min="0" max="16" value="6" oninput="updateSlider('sensitivity')">
                    <span id="sensitivityValue" class="slider-value">6</span>
                </div>
            </div>
...
exports.QonQreteSidebarProvider = QonQreteSidebarProvider;
