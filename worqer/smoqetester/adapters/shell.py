...
class ShellAdapter(Adapter):
    ...
    def _validation_python_path_prefix(self, ctx: SmoketestContext) -> str | None:
        """Return the task-local Python env bin dir when Python deps were provisioned."""
        try:
            python_bin, _ = provision_validation_env(ctx.qodeyard_path.parent, ctx.qodeyard_path)
        except Exception:
            return None
        if not python_bin:
            return None
    ...
    def _exercise_fastapi_contract(self, base_url: str) -> str | None:
        ...
        p1 = {"username": "alice", "email": "alice@example.com", "password": "password123"}
        ...
        if set(u1.keys()) != {"id", "username", "email"}:
            return f"POST /users unexpected fields: {sorted(u1.keys())}"

        p2 = {"username": "bravo", "email": "bravo@example.com", "password": "password456"}
    ...
    def _run_run_sh_launch_smoke(self, ctx: SmoketestContext, run_sh: Path, scope_files: list[Path]) -> list[SmoketestResult]:
        ...
        validation_path_prefix = self._validation_python_path_prefix(ctx)
        if validation_path_prefix:
            path_value = validation_path_prefix + (os.pathsep + path_value if path_value else "")
        env = {
            "PATH": path_value,
        ...
        try:
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    # Any HTTP response proves the process is reachable; exact contract
                    # behavior is verified by the dedicated behavioral probe below.
                    if code is not None:
                        ready = True
                        break
                except Exception:
