...
DECLARED_BY_TRANSITIVE_PROVIDER = {
    # FastAPI includes Pydantic in its dependency graph.
    "pydantic": {"fastapi"},
}
...
def _is_declared_or_transitively_covered(package: str, declared: set[str]) -> bool:
...
def _missing_python_declarations(qodeyard: Path, manifests: list[str]) -> list[str]:
    ...
    for root in _collect_python_import_roots(qodeyard):
        ...
        if declared and not _is_declared_or_transitively_covered(package, declared):
            missing.add(package)
