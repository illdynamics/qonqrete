...
def _dispatch_openai_compatible(provider: str, model: str, messages: list[dict[str, str]], output_tokens: int, timeout: int) -> DispatchResult:
...
def _dispatch_anthropic(model: str, messages: list[dict[str, str]], output_tokens: int, timeout: int) -> DispatchResult:
...
def _dispatch_gemini(model: str, messages: list[dict[str, str]], output_tokens: int, timeout: int) -> DispatchResult:
...
def run_ai_messages(
    ...
    try:
        if provider_l in {"openai", "deepseek", "qwen", "openrouter"}:
            return _dispatch_openai_compatible(provider_l, model, messages, output_tokens, resolved_timeout)
...
def _dispatch_with_chunking(provider: str, model: str, inline_prompt: str, chunks: list[ChunkRecord], output_tokens: int, timeout: int | None = None) -> DispatchResult:
    ...
    for preload in _build_preload_messages(chunks):
        ...
        ack_result = run_ai_messages(provider, model, preload_messages, output_tokens=128, timeout=timeout)
        ...
        if ack_result.text.strip() != expected:
            raise RuntimeError(f"Chunk preload ACK mismatch. Expected `{expected}` got `{ack_result.text.strip()}`")
    ...
    final_result = run_ai_messages(provider, model, messages + [final_user], output_tokens=output_tokens, timeout=timeout)
...
def _prepare_request_plan(
    ...
    caps = resolve_model_capabilities(capability_provider, model, config=config)
