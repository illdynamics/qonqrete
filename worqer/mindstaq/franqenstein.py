#!/usr/bin/env python3
"""
Franqenstein Combinator: Smart Code Snippet Merger
...
class FranqensteinCombinator:
    ...
    Usage:
        combinator = FranqensteinCombinator()
    ...
    def score_component(self, comp: CodeComponent) -> float:
        ...
        v2.2.7: Added COPYPASTA PENALTIES for generic class names!
        """
        ...
        # v2.2.7: COPYPASTA PENALTIES!
        # Generic class names indicate template copypasta, NOT task-specific code
        generic_names = {
            'repository', 'config', 'shared', 'error', 'entity',
            'base', 'generic', 'abstract', 'handler', 'manager',
            'service', 'controller', 'model', 'helper', 'utils',
        }
        
        if comp.name and comp.name.lower() in generic_names:
            score -= 0.4  # Heavy penalty for generic names
        
        # v2.2.7: Check code content for copypasta indicators
        code_lower = comp.code.lower() if comp.code else ''
        ...
        if copypasta_count >= 2:
            score -= 0.5  # Heavy penalty for obvious copypasta
        elif copypasta_count >= 1:
            score -= 0.3
        
        # v2.2.7: BONUS for tool-specific code
        tool_indicators = [
            'subprocess.run', 'subprocess.popen',
            'nmap', 'bloodhound', 'feroxbuster', 'masscan',
            'neo4j', 'cypher', 'ldap', 'kerberos',
            'socket.socket', '-ox', '-on', '-og',  # nmap output formats
        ]
        
        tool_count = sum(1 for ind in tool_indicators if ind in code_lower)
        if tool_count >= 2:
            score += 0.4  # Big bonus for tool-specific code
        elif tool_count >= 1:
            score += 0.2
...
if __name__ == '__main__':
    print("=" * 60)
    print(f"Franqenstein Combinator v{__version__}")
    print("=" * 60)
...
''',
    ...
    print("\n✅ Franqenstein Combinator working!")
