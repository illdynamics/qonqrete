...
def main() -> None:
    ...
    provider_options = lib_provider_config.resolve_agent_provider_options(config, 'instruqtor')
    agent_cfg = config.get('agents', {}).get('instruqtor', {})
    ai_provider = provider_options.get('provider') or agent_cfg.get('provider', 'openai')
    ai_model = provider_options.get('model') or agent_cfg.get('model', 'gpt-4o')
