...
# v1.2.4-stable - QONTRACT Generation + Invariant Injection
# ═══════════════════════════════════════════════════════════════════════════════
#
# v1.2.4: BATCHED BRIQ GENERATION! 🚀
...
try:
    ...
    from mode_policy import (
        ...
        classify_planned_briq,
        ensure_scope_class_line,
        is_optional_scope,
        load_mode_policy_from_env,
        render_instruqtor_directives,
    )
...
# ═══════════════════════════════════════════════════════════════════════════════
# QONTRACT GENERATION (v1.2.4) - Persistent Constitution from Cycle 1
# ═══════════════════════════════════════════════════════════════════════════════
...
def generate_qontract_md(contract: dict) -> str:
    ...
    md += f"_Generated by InstruQtor v1.2.4 | Source: cycle {meta.get('source_cycle', 1)} | Hash: {meta.get('hash', 'n/a')}_\n\n"
    md += "---\n\n"
...
# ═══════════════════════════════════════════════════════════════════════════════
# INVARIANT INJECTION INTO BRIQS (v1.2.4)
# ═══════════════════════════════════════════════════════════════════════════════
...
"""
        ...
        # Call AI
        try:
            # Get AI timeout and provider options
            ai_timeout = agent_cfg.get('timeout')
            ai_provider_options = agent_cfg.get(ai_provider) if ai_provider == 'llamacpp' else None
...
def main() -> None:
    ...
    # v1.2.4: Load batched generation config
    ...
    # v1.2.4: Determine if we should use batched generation
...
"""
    ...
    # v1.0.5: Qrystallizer Artifact Loading
    req_context = ""
    enhancement_context = ""
    qrystal_reqs = []
    worqspace_root = Path(os.environ.get('QONQ_WORKSPACE', os.getcwd()))
    qrystal_dir = worqspace_root / 'qrystal.d'
    req_path = qrystal_dir / 'requirements.json'
    if req_path.exists():
        try:
            with open(req_path, 'r', encoding='utf-8') as f:
                qdata = json.load(f)
                qrystal_reqs = qdata.get('requirements', [])
                if qrystal_reqs:
                    req_context = "\n**STABLE REQUIREMENTS (MAP BRIQS TO THESE IDs):**\n"
                    for r in qrystal_reqs:
                        req_context += f"- [{r.get('id', 'N/A')}] {r.get('text', '')}\n"
            print(f"  💎 [QRYSTALLIZER] Loaded {len(qrystal_reqs)} requirements from {qrystal_dir.name}", flush=True)
        except Exception as e:
            print(f"  ⚠️  [QRYSTALLIZER] Could not load requirements: {e}", flush=True)

    enhancement_path = qrystal_dir / 'enhancement_backlog.json'
    if mode_policy.allow_optional_enhancements and enhancement_path.exists():
        try:
            enhancement_data = json.loads(enhancement_path.read_text(encoding='utf-8'))
            enhancements = enhancement_data.get('enhancements', [])
            if enhancements:
                enhancement_context = "\n**OPTIONAL ENHANCEMENT BACKLOG (KEEP OPTIONAL):**\n"
                for item in enhancements[:12]:
                    if isinstance(item, dict):
                        enhancement_context += f"- {item.get('title', 'Untitled')}: {item.get('reason', '')}\n"
                    else:
                        enhancement_context += f"- {item}\n"
                print(f"  💡 [QRYSTALLIZER] Loaded {len(enhancements)} optional enhancements", flush=True)
        except Exception as e:
            print(f"  ⚠️  [QRYSTALLIZER] Could not load enhancement backlog: {e}", flush=True)

    mode_instructions = render_instruqtor_directives(mode_policy)
...
6.  **NO SCOPE CREEP:** Never turn optional ideas into mandatory execution scope.
...
Each briq body MUST begin with these lines in this exact order:
`Requirement-IDs: REQ-001, REQ-002`
`Scope-Class: MANDATORY`
If a briq is purely supporting work with no direct requirement mapping, use:
`Requirement-IDs: NONE`
In `innovative` mode, optional enhancement briqs MUST use:
`Requirement-IDs: NONE`
`Scope-Class: OPTIONAL_ENHANCEMENT`

When stable requirements are available, map every mandatory briq to the most relevant requirement IDs. Optional enhancements must stay separate from the canonical requirement ledger.
...
"""
    ...
    # v1.2.4: Generate briqs using appropriate strategy
    if use_batched:
    ...
    normalized_briqs = []
    for item in briqs:
        scope_class = classify_planned_briq(mode_policy, item.get('title', ''), item.get('content', ''))
        item['scope_class'] = scope_class
        item['content'] = ensure_scope_class_line(item.get('content', ''), scope_class)
        if mode_policy.freeze_execution_scope and is_optional_scope(scope_class):
            deferred_enhancements.append({
                'title': item.get('title', 'Untitled'),
                'reason': item.get('content', '').splitlines()[0] if item.get('content') else 'Deferred optional idea',
                'promotion_trigger': 'Explicitly promote this idea into the canonical requirement ledger.'
            })
            continue
        normalized_briqs.append(item)
    if deferred_enhancements:
        print(f"  ⚠️  [MODE] Deferred {len(deferred_enhancements)} enhancement briqs because mode=program", flush=True)
    briqs = normalized_briqs
    print(f"  🎛️ [MODE] Active semantics: {mode_policy.semantic_mode} | execution briqs: {len(briqs)}", flush=True)

    # ═══════════════════════════════════════════════════════════════════════════
    # v1.2.4: QONTRACT GENERATION (Cycle 1 only)
    # ═══════════════════════════════════════════════════════════════════════════
    worqspace_root = Path(os.environ.get('QONQ_WORKSPACE', os.getcwd()))
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # WRITE BRIQS (with v1.2.4 invariant injection)
    # ═══════════════════════════════════════════════════════════════════════════
    for i, item in enumerate(briqs):
        ...
        # v1.2.4: Inject relevant invariants based on scope tags + add Contract-Relevant header
        scope_tags = []
        ...
        # v1.2.4: Build briq with frontmatter including scope + contract relevance
        scope_str = ', '.join(scope_tags) if scope_tags else 'none'

        req_ids_match = re.search(r'^Requirement-IDs:\s*(.+)$', briq_content, re.IGNORECASE | re.MULTILINE)
        req_ids_raw = req_ids_match.group(1).strip() if req_ids_match else 'NONE'
        req_ids = [rid.strip() for rid in req_ids_raw.split(',') if rid.strip()] if req_ids_raw.upper() != 'NONE' else []
        scope_class = item.get('scope_class', MANDATORY_SCOPE_CLASS)
        if is_optional_scope(scope_class):
            req_ids = []
        req_ids_str = ', '.join(req_ids) if req_ids else 'NONE'

        frontmatter = (
            f"Scope: {scope_str}\n"
            f"Scope-Class: {scope_class}\n"
            f"Contract-Relevant: {'yes' if contract_relevant else 'no'}\n"
            f"Requirement-IDs: {req_ids_str}\n\n"
        )
