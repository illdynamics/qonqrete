...
def process_briq_interleaved(
    ...
    ai_provider_options: dict | None = None,
    review_provider_options: dict | None = None,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            ai_result = lib_ai.run_ai_completion(
                ...
                timeout=(ai_provider_options or {}).get('timeout'),
                request_options=ai_provider_options,
            )
            ...
            if do_ai_review and build_passed:
                ...
                review = run_ai_quick_review(
                    ...
                    provider_options=review_provider_options or ai_provider_options,
                )
...
def main():
    ...
    construqtor_options = lib_provider_config.resolve_agent_provider_options(config, 'construqtor')
    agent_cfg = config.get('agents', {}).get('construqtor', {})
    ai_provider = construqtor_options.get('provider') or agent_cfg.get('provider', 'gemini')
    ai_model = construqtor_options.get('model') or agent_cfg.get('model', 'gemini-1.5-pro')
    use_qompressor = config.get('options', {}).get('use_qompressor', True)
    
    # InspeQtor config for reviews
    inspeqtor_options = lib_provider_config.resolve_agent_provider_options(config, 'inspeqtor')
    inspeqtor_cfg = config.get('agents', {}).get('inspeqtor', {})
    review_provider = inspeqtor_options.get('provider') or inspeqtor_cfg.get('provider', ai_provider)
    review_model = inspeqtor_options.get('model') or inspeqtor_cfg.get('model', ai_model)
    ...
    for briq_file in briq_files:
        ...
        result = process_briq_interleaved(
            ...
            review_provider,
            review_model,
            ai_provider_options=construqtor_options,
            review_provider_options=inspeqtor_options,
