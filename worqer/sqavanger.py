#!/usr/bin/env python3
"""
sQavanger: Search Harvester Agent (Tier 1)
...
class SQavanger:
    ...
    def __init__(self, config: dict = None):
        ...
        sqavanger_cfg = self.config.get('sqavanger', self.config.get('mindstaq', {}))
        qrawler_cfg = self.config.get('qrawler', sqavanger_cfg.get('qrawler', {}))
        
        # Qrawler configuration
        self.qrawler_enabled = qrawler_cfg.get('enabled', True)  # Enabled by default!
        self.qrawler_url = qrawler_cfg.get('url', 'http://localhost:8888')
        ...
        if HAS_QRAWLER and self.qrawler_enabled:
            try:
                ...
                sys.stderr.write(f"[sQavanger] Qrawler init failed: {e}\n")
        
        # Configuration
        self.max_results = sqavanger_cfg.get('max_results', 10)
        self.min_relevance = sqavanger_cfg.get('min_relevance', 0.3)
        self.min_quality = sqavanger_cfg.get('min_quality', 0.5)
    ...
    def _search_web(self, intent: CrystallizedIntent, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            sys.stderr.write(f"[sQavanger] Web search: {query[:50]}...\n")
            
            # Search
            result = self.qrawler.search_sync(query, max_results=self.max_results)
            
            if not result.code_snippets:
                sys.stderr.write(f"[sQavanger] No code snippets found\n")
                return None
            
            sys.stderr.write(f"[sQavanger] Found {len(result.code_snippets)} snippets from {result.engines_used}\n")
            ...
            if not scored:
                sys.stderr.write(f"[sQavanger] No snippets passed quality/relevance threshold\n")
                return None
            ...
            sys.stderr.write(f"[sQavanger] Best match: relevance={best[2]:.2f}, quality={best[3]:.2f}\n")
            
            return best[0].code
            
        except Exception as e:
            sys.stderr.write(f"[sQavanger] Web search error: {e}\n")
            return None
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='sQavanger - Search Harvester Agent')
    ...
    if args.list:
        ...
        sqavanger = SQavanger()
        code = sqavanger.harvest(intent, args.text)
