...
def main() -> None:
    if len(sys.argv) != 3: print("Usage: inspeqtor.py <input> <output>"); sys.exit(1)
    ...
    try: with open(summary_path, 'r', encoding='utf-8') as f: summary_content = f.read()
    except FileNotFoundError: print(f"CRITICAL: Summary missing", flush=True); sys.exit(1)

    try: with open('config.yaml', 'r') as f: config = yaml.safe_load(f) or {}
    except: config = {}
    ...
    if qodeyard_path.is_dir():
        for root, _, files in os.walk(qodeyard_path):
            for name in files:
                ...
                try: with open(fpath, 'r', encoding='utf-8') as f: context_str += f"\n### File: `{rel_path}`\n```\n{f.read()}\n```\n"
                except: pass
...
"""
    ...
    try:
        ...
        print(f"Inspeqtor Failure: {e}", flush=True); sys.exit(1)
