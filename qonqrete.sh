...
case "$COMMAND" in
    ...
    clean)
        log_qrane "Cleaning QonQrete worqspace..."
        if [ -d "${WORKSPACE_DIR}" ]; then
            PROMPT_STR="${PREFIX_TPL/\{PREFIX\}/_QQ} Delete contents of qodeyard, briq.d, reqap.d, exeq.d, and struqture? [y/N] "
            echo -ne "$PROMPT_STR"
            read -n 1 -r
            echo # Print newline after input

            if [[ $REPLY =~ ^[Yy]$ ]]; then
                rm -rf "${WORKSPACE_DIR}"/qodeyard/*
                rm -rf "${WORKSPACE_DIR}"/briq.d/*
                rm -rf "${WORKSPACE_DIR}"/reqap.d/*
                rm -rf "${WORKSPACE_DIR}"/exeq.d/*
                rm -rf "${WORKSPACE_DIR}"/struqture/*
    ...
    run)
        ...
        log_qrane "Seeding worQspace in Qage at: ${WORKSPACE_DIR}"

        mkdir -p "$WORKSPACE_DIR"/{tasq.d,exeq.d,reqap.d,qodeyard,struqture,sqrapyard}

        if [ ! -f "${WORKSPACE_DIR}/tasq.d/cyqle1_tasq.md" ]; then
            if [ -f "${WORKSPACE_DIR}/tasq.md" ]; then
                cp "${WORKSPACE_DIR}/tasq.md" "$WORKSPACE_DIR/tasq.d/cyqle1_tasq.md"
            else
                echo "Create a simple Python script." > "$WORKSPACE_DIR/tasq.d/cyqle1_tasq.md"
            fi
        fi

        DEV_MOUNTS="-v ${SCRIPT_DIR}/qrane:/qonqrete/qrane -v ${SCRIPT_DIR}/worqer:/qonqrete/worqer"
        RUN_MOUNTS="-v ${WORKSPACE_DIR}:${CONTAINER_WORKSPACE}"
        ...
        if [ "$RUNTIME_MODE" == "msb" ]; then
            CMD_BIN="msb"; if command -v mbx >/dev/null 2I> /dev/null; then CMD_BIN="mbx"; fi
