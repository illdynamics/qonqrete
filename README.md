...
Prerequisites:
- Docker or Podman
- Cloud AI API key(s) **or** a running host-side `llama-server` if you use `provider: llamacpp`
...
| llama.cpp | none by default (`LLAMACPP_API_KEY` optional) |
| `local` helper agents | not applicable |

`local` still means QonQrete's built-in helper-agent scripts such as `calqulator`, `qontextor`, `qompressor`, and `qontrabender`. It is **not** the llama.cpp path.

## llama.cpp provider quick example

Start `llama-server` on the **host Mac**, not inside the QonQrete container. A typical M1 Max launch looks like:
...
Then point containerized QonQrete at `http://host.docker.internal:8080/v1` on Docker Desktop, or `http://host.containers.internal:8080/v1` on Podman.
...
Default endpoint fallback order for `llamacpp` is:
`agents.<agent>.endpoint` → `providers.llamacpp.endpoint` → `LLAMACPP_ENDPOINT` / `QONQ_LLAMACPP_ENDPOINT` → `http://host.docker.internal:8080/v1` → `http://host.containers.internal:8080/v1` → `http://localhost:8080/v1`.

QonQrete treats `llamacpp` as a local / zero-dollar provider. Token estimates may still appear, but dollar estimates stay at `$0.00`. The shipped config validator allows `providers.llamacpp.timeout` and per-agent `timeout` values up to `900` seconds for this provider, while standard cloud providers stay capped at `300` seconds. Both IDE integrations can store an optional `LLAMACPP_API_KEY` securely for auth-fronted endpoints without making it mandatory for normal local usage. On a Mac M1 or M1 Max workflow, keep `llama-server` running on macOS itself and point the containerized QonQrete runtime at `host.docker.internal` (Docker Desktop) or `host.containers.internal` (Podman).

## Smoke tests for llama.cpp logic

```bash
python -m unittest discover -s tests -p 'test_*.py' -v
```

This lightweight suite covers llama.cpp endpoint normalization/fallbacks, provider-option resolution, timeout validation, and model-id matching without requiring a real `llama-server` binary.
...
- Set **API keys** for each provider that needs one (stored securely)
- See at a glance which keys are set and which are missing

Changes are written directly to `.qonqrete/worqspace/config.yaml`.

After deploying QonQrete to a workspace, the IDE will prompt you to configure AI providers and any API keys required by the selected providers.
...
export LLAMACPP_ENDPOINT='http://host.docker.internal:8080/v1'   # optional override
export QONQ_LLAMACPP_ENDPOINT='http://host.docker.internal:8080/v1'
export LLAMACPP_API_KEY='...'   # optional, only if your endpoint is fronted by auth
# llama.cpp uses no cloud API key by default
```

`provider: llamacpp` is a separate remote-LLM path that talks over HTTP to an already-running `llama-server`. `provider: local` remains reserved for helper-agent script routing inside `qrane/qrane.py`.
